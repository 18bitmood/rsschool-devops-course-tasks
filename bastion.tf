...
resource "aws_instance" "bastion" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ssh -i /home/ubuntu/.ssh/${var.bastion_key_name}.pem -o StrictHostKeyChecking=no ubuntu@${aws_instance.ubuntu_public_instances[0].private_ip} 'curl -sfL https://get.k3s.io -o k3s-install.sh && sudo sh k3s-install.sh'",
      "sleep 120",

      # Retrieve the node token and save it to a file
      "ssh -i /home/ubuntu/.ssh/${var.bastion_key_name}.pem -o StrictHostKeyChecking=no ubuntu@${aws_instance.ubuntu_public_instances[0].private_ip} 'sudo cat /var/lib/rancher/k3s/server/node-token' > /home/ubuntu/k3s_token.txt",

      # Install k3s in agent mode on other public instances
      "${join("\n",
        flatten([
          for ip in slice(aws_instance.ubuntu_public_instances[*].private_ip, 1, length(aws_instance.ubuntu_public_instances)) : [
            ...
            "ssh -i /home/ubuntu/.ssh/${var.bastion_key_name}.pem -o StrictHostKeyChecking=no ubuntu@${ip} 'K3S_URL=https://${aws_instance.ubuntu_public_instances[0].private_ip}:6443 K3S_TOKEN=$(cat /home/ubuntu/k3s_token.txt) sh k3s-install.sh agent &'"
          ]
        ])
      )}",
      "sleep 150"
